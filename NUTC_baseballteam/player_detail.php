...
$performances = array_filter($db->getAll('player_performance'), function($p) use ($player_id) {
    ...
    <div class="container">
        <div class="player-detail-layout">
            ...
            <div class="player-portrait-card">
                <img src="<?= htmlspecialchars($player['image_path']) ?>" alt="<?= htmlspecialchars($player['name']) ?>" class="player-portrait-img">
                <div class="player-portrait-info">
                    <h2 class="player-portrait-name"><?= htmlspecialchars($player['name']) ?></h2>
                    ...
                    <table class="player-records-table">
                        ...
                        <tbody>
                            <?php foreach ($performances as $perf): ?>
                                <tr>
                                    <td style="font-weight: 700;">比賽 ID: <?= $perf['game_id'] ?></td>
                                    <td><?= $perf['hit'] ?></td>
                                    <td><?= $perf['rbi'] ?></td>
                                    <td><?= $perf['runs'] ?></td>
                                    <td><?= $perf['at_bats'] ?></td>
